...
# Check if gpte is installed
def is_gpte_installed():
    """Check if GPT-Engineer command-line tool is installed."""
    try:
        return shutil.which('gpte') is not None
    except:
        return False

# Display warning if gpte is not installed
if not is_gpte_installed():
    logger.error("GPT-Engineer (gpte) command not found in PATH. Please install it.")
...
app.config.from_mapping(
    ...
    GPT_MODELS=json.loads(os.getenv('GPT_MODELS', '{}')),
    API_CONFIG={
...
def run_gpt_engineer(project_dir, model=None, task_type="create", callback=None, metadata=None, user_id=None):
    """
    Run GPT-Engineer process with proper Flask application context handling.
    ...
    """
    # First check if gpte is installed
    if not is_gpte_installed():
        error_msg = "GPT-Engineer (gpte) command not found. Please install it with 'pip install gpt-engineer'."
        logger.error(error_msg)
        flash(error_msg, 'danger')
        return
    
    # Function to run within app context
    def execute_gpte_in_context():
        with app.app_context():
            try:
                ...
                if project:
                    job = JobHistory(
                        ...
                        job_type=task_type,
                        model=model if model else app.config['API_CONFIG']['MODEL_NAME'],
    ...
    try:
        # Create and start a thread that runs the function within the app context
        thread = threading.Thread(target=execute_gpte_in_context)
        thread.daemon = True
        thread.start()
    except Exception as e:
        logger.exception(f"Error starting GPT-Engineer thread: {str(e)}")
        flash(f"Failed to start project generation: {str(e)}", 'danger')
...
def create_project():
    ...
    if request.method == 'POST':
        ...
        model = request.form.get('model', app.config['API_CONFIG']['MODEL_NAME'])
    ...
    # If no models defined in config, use the default from API_CONFIG
    if not models:
        default_model = app.config['API_CONFIG']['MODEL_NAME']
        models.append({
            'id': default_model,
            'name': default_model,
...
def api_create_project_impl():
    ...
    try:
        ...
        project = Project(
            ...
            prompt=data['prompt'],
            model=data.get('model', app.config['API_CONFIG']['MODEL_NAME']),
            status='pending',
            tags=data.get('tags', ''),
...
def api_improve_project_impl(project_id):
    ...
    try:
        ...
        project.model = data.get('model', app.config['API_CONFIG']['MODEL_NAME'])
        project.status = 'pending'
        project.last_modified = datetime.utcnow()
        db.session.commit()
...
def api_key_check_project_access(project):
    """Check if the API key in the request has access to this project"""
    if not hasattr(g, 'current_user'):
        return False
        
    return g.current_user.is_admin or project.user_id == g.current_user.id or project.is_public


# Helper function for model descriptions
def get_model_description(model_id):
    """Get description for a model"""
    # Use a simple default description since specific GPT models are removed
